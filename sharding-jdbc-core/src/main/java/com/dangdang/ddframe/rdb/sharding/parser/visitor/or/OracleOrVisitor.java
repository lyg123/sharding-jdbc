...
import com.dangdang.ddframe.rdb.sharding.parser.visitor.basic.oracle.AbstractOracleVisitor;
 ...
 * @author gaohongtao
 */
public class OracleOrVisitor extends AbstractOracleVisitor implements OrVisitor{
    
    private AbstractOrASTNode orASTNode;
    
    public OracleOrVisitor(final SQLASTOutputVisitor dependencyVisitor) {
    ...
    public boolean visit(final SQLBinaryOpExpr x) {
        ...
        if (orASTNode == null) {
            orASTNode = new SimpleOrASTNode(x, new OracleOrVisitor(this));
        } else {
            CompositeOrASTNode existingOutConditionOrASTNode = new CompositeOrASTNode();
            existingOutConditionOrASTNode.addSubNode(orASTNode);
            existingOutConditionOrASTNode.addSubNode(new SimpleOrASTNode(x, new OracleOrVisitor(this)));
            orASTNode = existingOutConditionOrASTNode;
        }
        return false;
    }
