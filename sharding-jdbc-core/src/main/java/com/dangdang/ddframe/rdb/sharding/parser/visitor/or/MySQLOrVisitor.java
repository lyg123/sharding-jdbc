 ...
 * @author gaohongtao
 */
public class MySQLOrVisitor extends AbstractMySQLVisitor implements OrVisitor{
    
    private AbstractOrASTNode orASTNode;
    
    public MySQLOrVisitor(final SQLASTOutputVisitor dependencyVisitor) {
    ...
    public boolean visit(final SQLBinaryOpExpr x) {
        ...
        if (orASTNode == null) {
            orASTNode = new SimpleOrASTNode(x, new MySQLOrVisitor(this));
        } else {
            CompositeOrASTNode existingOutConditionOrASTNode = new CompositeOrASTNode();
            existingOutConditionOrASTNode.addSubNode(orASTNode);
            existingOutConditionOrASTNode.addSubNode(new SimpleOrASTNode(x, new MySQLOrVisitor(this)));
            orASTNode = existingOutConditionOrASTNode;
        }
        return false;
    }
